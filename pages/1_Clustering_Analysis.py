...
def load_clustering_data():
    try:
        # On récupère tout l'historique pour avoir une analyse solide
...
@st.cache_data
def load_geojson():
    # On charge le GeoJSON sans filtrer par 'area_type' pour inclure Surrey, Port Moody, etc.
    gdf = gpd.read_file('data/metro_vancouver_map.geojson')
    return gdf

# --- LOGIQUE PRINCIPALE ---
st.title("🤖 Segmentation Intelligente des Quartiers")
st.write("Analyse des profils de performance par zone géographique (K-Means Clustering).")
...
if not df_all.empty:
    ...
    # On baisse le seuil à 2 relevés pour être plus inclusif avec les villes périphériques
    stats = df_all.groupby('area_name')['delay_min'].agg(['mean', 'std', 'count']).reset_index()
    stats['std'] = stats['std'].fillna(0) # Pas de variation si 1 seul bus
    stats = stats[stats['count'] >= 2]

    if len(stats) > 3:
        # 2. MACHINE LEARNING : K-MEANS
        scaler = StandardScaler()
        # Normalisation des données pour que la moyenne et la variance aient le même poids
        ...
        # 3. JOINTURE GÉOGRAPHIQUE ROBUSTE
        # Nettoyage des noms (Majuscules + suppression des espaces inutiles)
        ...
        if missing_count > 0:
            with st.expander(f"🔍 Diagnostic : {missing_count} zones non cartographiées"):
                st.write("Ces zones sont calculées dans l'IA mais leurs noms ne correspondent pas au GeoJSON :")
        ...
        cluster_cols = st.columns(len(stats['cluster'].unique()))
        
        for idx, cluster_id in enumerate(sorted(stats['cluster'].unique())):
            c_data = stats[stats['cluster'] == cluster_id]
            with cluster_cols[idx]:
                st.metric(f"Cluster {cluster_id}", f"{len(c_data)} Zones")
                st.write(f"**Retard moyen:** {c_data['mean'].mean():.2f} min")
                st.write(f"**Instabilité:** {c_data['std'].mean():.2f}")

    else:
        st.info("Accumulez encore quelques données pour lancer l'algorithme (besoin de plus de quartiers avec au moins 2 relevés).")
else:
    st.warning("La base de données est vide. Lancez le scraper pour commencer l'analyse.")import streamlit as st
import pandas as pd
import geopandas as gpd
from sklearn.cluster import KMeans
from sklearn.preprocessing import StandardScaler
