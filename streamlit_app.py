...
# --- DATA LOADING (WITH OFF-MAP EXCLUSION) ---
...
def load_data():
    ...
    if not df.empty:
        # 1. EXCLUDE OFF-MAP VEHICLES IMMEDIATELY
        df = df[df['area_name'] != 'Off-Map'].copy()
        
        # 2. Safety drop for pagination overlaps
...
if not raw_df.empty:
    ...
    # Since Off-Map is already gone, we don't need to filter it here anymore
    area_stats = df.groupby('area_name')['delay_min'].mean()
    worst_area = area_stats.idxmax() if not area_stats.empty else "N/A"

    col1.metric("Buses On-Grid", unique_buses) # Renamed for clarity
...
st.caption("Data source: TransLink Open Data • All 'Off-Map' vehicles are excluded from this analysis.")import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import matplotlib.colors as mcolors
import seaborn as sns
